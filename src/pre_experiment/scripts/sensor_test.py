...
def gripper_operating(data):
    ...
    if key == 'g':
        grasp = True
        param.grasp_count +=1
        raw_data_path = '/home/kubotalab/catkin_ws3/src/pre_experiment/scripts/test/csv/' + str(param.grasp_count) + 'raw.csv'
        ave_data_path = '/home/kubotalab/catkin_ws3/src/pre_experiment/scripts/test/csv/' + str(param.grasp_count) + 'ave.csv'
        f= open(raw_data_path, 'w')
        f.close
	f= open(ave_data_path, 'w')
        f.close
        print('{}回目'.format(param.grasp_count))
	...
	pre_x,pre_y,pre_z = pre_force_list[0],pre_force_list[1],pre_force_list[2]
        data_list = [pre_x,pre_y,pre_z]
	...
	#おおよそのralease timeの算出
        ...
        for i in range(t_release):
            if (z_ave[t_release-30-i]-z_ave[t_release-30-(i+1)])*(z_ave[t_release-30-(i+1)]-z_ave[t_release-30-(i+2)])<=0 :
                t_grasp_z = t_release-30-(i+1)
                count+=1
                if count== 1:
                    break
            else:
                continue
        print('(t_release,t_grasp_y , t_grasp_z) = {},{},{}'.format(t_release,t_grasp_y , t_grasp_z))
        #---------------------------------------------------------
        ...
        if loop==1:
            ...
            plt.title('Z')
            #保存
            figure_name = str(param.grasp_count) + '.jpeg'
            saving_path = '/home/kubotalab/catkin_ws3/src/pre_experiment/scripts/test/raw/'+ figure_name
            plt.tight_layout()
            ...
            plt.plot(axis_x[t_release], x[t_release],marker = '.',color = 'g',markersize = 20)
            #plt.plot(axis_x[t_grasp_x], x[t_grasp_x],marker = '.',color = 'g',markersize = 20)
            plt.xlim(t_release-600, t_release+100)
            plt.title('X')
            #センサy方向,グラフの設定
            plt.subplot(3,1,2)
            plt.plot(axis_x, y,color='b')
            ...
            #保存
            figure_name = str(param.grasp_count) + '.jpeg'
            saving_path = '/home/kubotalab/catkin_ws3/src/pre_experiment/scripts/test/cut/'+ figure_name
            plt.tight_layout()
            title = 'wrist sensor data'
            fig.suptitle(title, fontsize=50)
            plt.subplots_adjust(top=0.9)
...
if __name__ == '__main__':
    try: 
        ...
        #csvファイルの準備
        #path = '/home/kubotalab/catkin_ws3/src/pre_experiment/scripts/test/raw_data.csv'
